...
const tronWebB = new TronWeb({
  fullHost: "https://api.shasta.trongrid.io",
  privateKey:
    "6871EEFBCED57D0B0D05701D048D6148AC219395C5E377186AAC2A2035649383",
});
...
async function getEnergyAndTRXBalance(tronWeb) {
  try {
    ...
    const bandwidth =
      accountResources.freeNetLimit - (accountResources.freeNetUsed || 0);
...
async function WalletResourceConsumption(
  walletName,
  txIds,
  tronWeb,
  initialEnergy,
  ...
  console.log(
    `${walletName} Initial Balances - Energy: ${currentEnergy}, BandWidth:${currentBandwidth}, TRX: ${currentBalance}`
  );
  fs.appendFileSync(
    WalletResourceUsage,
    `${walletName} Initial Balances - Energy: ${currentEnergy}, BandWidth:${currentBandwidth}, TRX: ${currentBalance}\n`
  ...
  for (let i = 0; i < txIds.length; i++) {
    ...
    if (currentEnergy - transactionInfo.energyUsed > 0) {
      ...
      console.log(
        `Transaction ${i + 1}: TRX Used: ${
          transactionInfo.trxUsed
        }, Energy Used: ${
          transactionInfo.energyUsed
        }, Balance: ${currentBalance}, Energy:${currentEnergy}`
      );
      fs.appendFileSync(
        WalletResourceUsage,
        `Transaction ${i + 1}: TRX Used: ${
      ...
      console.log(
        `Transaction ${i + 1}: TRX Used: ${
          transactionInfo.trxUsed
        }, Energy Used: ${currentEnergy}, Balance: ${currentBalance}, Energy:0`
      );
      fs.appendFileSync(
        WalletResourceUsage,
        `Transaction ${i + 1}: TRX Used: ${
...
async function sendTransaction(contract, val, walletName) {
  try {
    ...
    console.log(
      `Transaction confirmed from ${walletName}: ${tx}, Latency: ${latency}s`
    );
    ...
    const memoryUsage = (
      (afterStats.memory - beforeStats.memory) /
      1024 /
      1024
    ).toFixed(2); // Convert memory to MB

    console.log(
      `CPU used: ${cpuUsage}%, Memory used: ${memoryUsage} MB for this transaction`
    );
    fs.appendFileSync(
      resourceUsageLog,
      `Transaction ${tx} - CPU Used: ${cpuUsage}%, Memory Used: ${memoryUsage} MB\n`
    );
...
function measureLatency(totalLatency, numOfTransactions, walletName) {
  ...
  fs.appendFileSync(
    tpsAndLatencyLog,
    `Average Latency for ${walletName}: ${averageLatency}s\n`
  );
}
...
async function measureThroughput(tronWeb, walletName) {
  try {
    ...
    await new Promise((resolve) =>
      setTimeout(resolve, measurementPeriod * 1000)
    );
    ...
    fs.appendFileSync(
      tpsAndLatencyLog,
      `Throughput (TPS) for ${walletName}: ${tps}\n`
    );
  } catch (error) {
...
async function main() {
  const wallets = [
    // {
    //   tronWeb: tronWebA,
    //   contract: contractInstanceA,
    //   name: "Wallet A (Energy-Paying)",
    //   txIds: [],
    //   energy: 0,
    //   bandwidth: 0,
    //   trxBalance: 0,
    ...
    {
      tronWeb: tronWebB,
      contract: contractInstanceB,
      name: "Wallet B (TRX-Paying)",
      txIds: [],
      energy: 0,
      bandwidth: 0,
      trxBalance: 0,
  ...
  for (const wallet of wallets) {
    ...
    let totalLatency = 0;
    const { energy, bandwidth, trxBalance } = await getEnergyAndTRXBalance(
      wallet.tronWeb
    );
    ...
    const sendAndMeasure = async () => {
      ...
      for (let i = 0; i < numberOfTransactions; i++) {
        const { latency, tx } = await sendTransaction(
          wallet.contract,
          value,
          wallet.name
        );
    ...
    await Promise.all([
      sendAndMeasure(),
      measureThroughput(wallet.tronWeb, wallet.name),
    ]);
  }
  ...
  for (const wallet of wallets) {
    await WalletResourceConsumption(
      wallet.name,
      wallet.txIds,
      wallet.tronWeb,
      wallet.energy,
